            ...
            <div class="card-body">
                <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @if($reportData['مستحقات المستقلين'] == 0)
                                <span> المستقلين</span>
                                <span class="badge bg-primary rounded-pill">{{ number_format($reportData['مستحقات المستقلين'], 2) }} $</span>
                            @else
                                <a href="{{route('freelacers.details',[ app()->getLocale(),$reportData['id']])}}"><span> المستقلين</span></a>
                                <span class="badge bg-primary rounded-pill">{{ number_format($reportData['مستحقات المستقلين'], 2) }} $</span>
                            @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @if($reportData['عمولة المسوقين بالعمولة'] == 0)
                                    <span> المسوقين بالعمولة </span></a>
                                    <span class="badge bg-primary rounded-pill">{{ number_format($reportData['عمولة المسوقين بالعمولة'], 2) }} $</span>
                                @else
                                    <a href="{{route('marketing.details',[ app()->getLocale(),$reportData['id']])}}"><span> المسوقين بالعمولة </span></a>
                                        <span class="badge bg-primary rounded-pill">{{ number_format($reportData['عمولة المسوقين بالعمولة'], 2) }} $</span>
                                @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @if($reportData['عمولة وكيل المبيعات'] == 0)
                                    <span>  وكلاء المبيعات</span>
                                    <span class="badge bg-primary rounded-pill">{{ number_format($reportData['عمولة وكيل المبيعات'], 2) }} $</span>
                                @else

                                    <a href="{{route('salesAgent.details',[ app()->getLocale(),$reportData['id']])}}"><span>  وكلاء المبيعات</span></a>
                                    <span class="badge bg-primary rounded-pill">{{ number_format($reportData['عمولة وكيل المبيعات'], 2) }} $</span>
                                @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @if($reportData['عمولة مسؤول المبيعات'] == 0)
                                    <span>  مسؤولي المبيعات</span>
                                    <span class="badge bg-primary rounded-pill">{{ number_format($reportData['عمولة مسؤول المبيعات'], 2) }} $</span>
                                @else
                                     <a href="{{route('salesOfficer.details',[ app()->getLocale(),$reportData['id']])}}"><span>  مسؤولي المبيعات</span></a>
                                     <span class="badge bg-primary rounded-pill">{{ number_format($reportData['عمولة مسؤول المبيعات'], 2) }} $</span>
                                @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @if($reportData['إجمالي الميزانيات'] == 0)
                                    <span>  الميزانيات</span>
                                    <span class="badge bg-primary rounded-pill">{{ number_format($reportData['إجمالي الميزانيات'], 2) }} $</span>
                                @else
                                    <a href="{{route('setting.details',[ app()->getLocale(),$reportData['id']])}}"><span>  الميزانيات</span></a>
                                    <span class="badge bg-primary rounded-pill">{{ number_format($reportData['إجمالي الميزانيات'], 2) }} $</span>
                                @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
