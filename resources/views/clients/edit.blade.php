...
<div class="container-fluid">
    ...
    <select name="source" id="source" class="form-control px-15">
        <option value="">المصدر</option>
        @php
            $sources = ['الفرانشيز', 'المسؤول', 'الوكيل', 'التسويق بالعمولة'];
        @endphp

        @foreach ($sources as $source)
            <option value="{{ $source }}" {{ $client->source == $source ? 'selected' : '' }}>
                {{ $source }}
            </option>
        @endforeach
    </select>
