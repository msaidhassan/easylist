                    ...
                    <div class="userDatatable global-shadow border-0 bg-white w-100">
                        <div class="table-responsive">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h4>{{ trans('menu.add_transfer_method') }}</h4>
                                <a href="{{ route('TransfersMethods.all', app()->getLocale()) }}" class="btn text-white add-order-btn">
                                    {{ trans('menu.back_to_list') }}
                                </a>
                            </div>

                            <!-- Add Transfer Method Form -->
                            <form action="{{ route('TransfersMethods', app()->getLocale()) }}" method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-25">
                                        <input type="text" class="form-control ih-medium ip-gray radius-xs b-light px-15" placeholder="اسم وسيله التحويل" name="name" required>
                                    </div>
                                    <div class="col-md-12">
